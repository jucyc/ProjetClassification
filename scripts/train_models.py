"""
Entrainement du modele lineaire (Perceptron / Rosenblatt) sur le dataset
monuments, via NOTRE bibliotheque C (python_api/ml_bridge.py -> lib C).

IMPORTANT : ce script n'implemente plus le modele lui-meme en Python.
Avant, ce fichier contenait une deuxieme version (en numpy pur) du
modele lineaire, en parallele de lib/src/linear_model.c. Le syllabus
interdit d'utiliser une implementation externe/parallele des
algorithmes vus en cours pour produire nos resultats : on appelle donc
desormais notre lib C via le bridge ctypes, comme pour tous les autres
modeles du projet.

Les fonctions de graphiques (matplotlib, sklearn.metrics pour les scores
de precision/rappel/F1) restent : ce ne sont que des outils de mesure et
de visualisation, pas une reimplementation de l'algorithme de classification.
"""
import numpy as np
import sys
import os
...
def generate_all_graphs(X_test, y_test, y_pred, class_names, y_all):
    ...
    # Note : pas de courbe de loss ici (le Perceptron/Rosenblatt n'a pas de
    # fonction de cout du type cross-entropy ; ce qu'on pourrait tracer a
    # la place, c'est l'evolution de l'accuracy au fil des iterations si
    # besoin pour le rapport).
...
def train_linear_model():
    ...
    # Entraînement (regle de Rosenblatt, one-vs-rest, via notre lib C)
    ...
    # Sauvegarde (utilise linear_save de notre lib C, pas np.savez)
